...
from dl_utils import compute_accuracy, compute_loss, compute_multilabel_accuracy
...
class Trainer:
    ...
    def __init__(
        self,
        data_dir: str,
        model: Union[SimpleNet, MyResNet18, OptimalNet],
        optimizer: Optimizer,
        model_dir: str,
    ...
    def validate(self) -> Tuple[float, float]:
        ...
        for (x, y) in self.val_loader:
            ...
            batch_loss = compute_loss(self.model, logits, y, is_normalize=True)
            # finding images where the first class is predicted incorrectly
            # for i in range(n):
            #     if y[i] != torch.argmax(logits[i]) and y[i] == 0:
            #         print(
            #             f"Image {i} is misclassified. Predicted class: {torch.argmax(logits[i])}"
            #         )
